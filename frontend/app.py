"""Toronto 311 City Services Assistant – Streamlit Frontend."""
import re
...
def create_conversation():
    d = api_post("/conversations",
                 {"title": f"Session {datetime.now().strftime('%b %d %H:%M')}"})
    return d.get("id") if d and "id" in d else None


def send_message(cid, msg):
    d = api_post(f"/conversations/{cid}/messages",
                 {"conversation_id": cid, "message": msg})
    return d if d and "intent" in d else None
...
# ── Session state ─────────────────────────────────────────────────────────────
for k, v in [("conversation_id", None), ("messages", []),
             ("quick_send", None), ("backend_ok", None)]:
...
backend_ok = st.session_state.backend_ok


# ── Top nav ───────────────────────────────────────────────────────────────────
status_html = (
    '<span class="pulse-dot"></span>System Online'
    if backend_ok else "🔴 Backend Offline"
)
st.markdown(f"""
<div class="top-nav">
  <div class="nav-left">
...
<div class="sb-section">
  ...
  <div class="sb-stat-row">
    ...
    <div class="sb-stat">
      <div class="sb-stat-v">{'🟢' if backend_ok else '🔴'}</div>
...
if not backend_ok:
    st.markdown("""
<div style="max-width:680px;margin:4rem auto;text-align:center;padding:2rem;background:#F8FAFC">
  <div style="font-size:2.5rem;margin-bottom:.75rem">⚠️</div>
  <h2 style="color:#1E293B;margin-bottom:.4rem">Backend Offline</h2>
  <p style="color:#64748B;font-size:1rem;margin-bottom:1rem">Start the backend server first:</p>
  <code style="background:#E8F4FD;border:1px solid #BFDBFE;padding:.6rem 1.3rem;
               border-radius:8px;display:inline-block;font-size:.9rem;color:#1E3A8A">
    python -m uvicorn backend.main:app --host 0.0.0.0 --port 8000 --reload
  </code>
  <p style="color:#94A3B8;font-size:.85rem;margin-top:1rem">Then refresh this page.</p>
</div>""", unsafe_allow_html=True)
    st.stop()
